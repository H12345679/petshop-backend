...
public class ProductServiceImpl extends ServiceImpl<ProductMapper, Product>
    ...
    /** 跨服务调用：会员折扣在 user 服务，经 OpenFeign 远程获取 */
    @Autowired
    private com.petshop.api.client.MembershipClient membershipClient;
    @Autowired
    private com.petshop.shop.mapper.ShopMapper shopMapper;
    /** 跨服务调用：个性化推荐在 recommend 服务，经 OpenFeign 远程获取（调用处已有异常兜底） */
    @Autowired
    private com.petshop.api.client.RecommendClient recommendClient;
    ...
    private void applyDiscountAndShopName(List<Product> list) {
        ...
        Long currentUserId = com.petshop.security.UserContext.getUserId();
        BigDecimal discount = membershipClient.getUserDiscount(currentUserId);
        String levelName = membershipClient.getUserLevelName(currentUserId);
        for (Product p : list) {
    ...
    private List<Product> recommendProducts(int n, Long userId) {
        ...
        try {
            recommendList = recommendClient.rankForUser(userId, n);
        } catch (Exception e) {
            // 排序服务异常不影响首页，走兜底
        }
