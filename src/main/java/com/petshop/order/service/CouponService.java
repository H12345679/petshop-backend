...
public interface CouponService {
    ...
    /** 后台新增优惠券（ADMIN/MERCHANT） */
    void createCoupon(Coupon coupon);

    /** 后台编辑/上下架（ADMIN/MERCHANT，MERCHANT 仅能操作自己店铺的券） */
    void updateCoupon(Long id, Coupon coupon);

    /** 后台分页列表（ADMIN 看全部，MERCHANT 只看自己店铺） */
    Page<Coupon> managePage(int current, int size, String name);
