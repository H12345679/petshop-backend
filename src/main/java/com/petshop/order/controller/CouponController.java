...
public class CouponController {
    ...
    @ApiOperation("后台新增优惠券（ADMIN/MERCHANT，商家自动绑定店铺）")
    ...
    @ApiOperation("后台编辑/上下架（ADMIN/MERCHANT，商家仅能操作自己的券）")
    @RequireRole({"ADMIN", "MERCHANT"})
    ...
    @ApiOperation("后台优惠券分页列表（ADMIN 看全部，MERCHANT 只看自己店铺）")
    @RequireRole({"ADMIN", "MERCHANT"})
    ...
    public Result<Map<String, Object>> managePage(
        ...
        Page<Coupon> page = couponService.managePage(current, size, name);
