...
import com.petshop.api.client.UserPetClient;
...
public class RecommendRankServiceImpl implements RecommendRankService {
    ...
    /** 跨服务调用：宠物档案在 user 服务，经 OpenFeign 远程获取 */
    @Autowired
    private UserPetClient userPetClient;
    ...
    public List<Product> rankForUser(Long userId, int n) {
        ...
        Set<Long> petTagIds = tagIdsByNames(userPetClient.petTagNames(userId));
        List<Integer> mySpecies = userPetClient.petSpecies(userId);
